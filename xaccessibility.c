...
static HRESULT WINAPI x_accessibility_XHighContrastGetMode( IXAccessibilityImpl2 *iface, XHighContrastMode *mode )
{
    ...
    if (!mode) return E_INVALIDARG;
    *mode = XHighContrastMode_Off;
    return S_OK;
}
...
static HRESULT WINAPI x_accessibility_XSpeechSynthesizerCreate( IXAccessibilityImpl2 *iface, XSpeechSynthesizerHandle *speechSynthesizer )
{
    ...
    if (!speechSynthesizer) return E_POINTER;
    *speechSynthesizer = (XSpeechSynthesizerHandle)0x3001;
    return S_OK;
}
...
{
    TRACE( "iface %p, speechSynthesizer %p, text %s, speechSynthesisStream %p\n", iface, speechSynthesizer, debugstr_a( text ), speechSynthesisStream );
    if (!speechSynthesisStream) return E_POINTER;
    *speechSynthesisStream = (XSpeechSynthesizerStreamHandle)0x3002;
    return S_OK;
}
...
{
    TRACE( "iface %p, speechSynthesisStream %p, bufferSize %p\n", iface, speechSynthesisStream, bufferSize );
